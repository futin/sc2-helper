...
## Corpus Check
- 43 files · ~14,622 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 441 nodes · 560 edges · 68 communities (29 shown, 39 thin omitted)
- Extraction: 76% EXTRACTED · 24% INFERRED · 0% AMBIGUOUS · INFERRED: 137 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1273ffef`
...
Cohesion: 0.09
Nodes (6): SC2HelperApp, main(), Entry point for the SC2 Helper UI., Start the backend process. Returns False if already running., RunnerController, RunnerScreen

### Community 4 - "Config & Service Layer"
Cohesion: 0.08
Nodes (8): get_mouse_pos(), Move mouse over each SC2 HUD element. Press Enter in terminal to record position, get_config(), Service layer — the only entry point for frontend to access config and stats. Fr, save_config(), CoordsTab, Smoke tests for config load/save round-trips., test_save_and_load_roundtrip()
...
## Knowledge Gaps
- **130 isolated node(s):** `Dev runner: watches src/ for .py changes and auto-restarts the app.`, `Smoke tests for detector logic — no TTS subprocess spawned.`, `Smoke tests for SpeechQueue — no actual TTS subprocess spawned.`, `Lower priority number = higher urgency = dequeued first.`, `Smoke tests for message selection logic.` (+125 more)
...
- **Why does `main()` connect `Game State Detectors` to `Config & Service Layer`, `Game API & HUD Capture`, `Game History Tracking`, `TTS Speech Queue`, `Voice Listener`?**
  _High betweenness centrality (0.198) - this node is a cross-community bridge._
- **Why does `get_config()` connect `Config & Service Layer` to `Game State Detectors`, `Frontend Application`, `Game API & HUD Capture`?**
  _High betweenness centrality (0.138) - this node is a cross-community bridge._
- **Why does `GameHistoryManager` connect `Game History Tracking` to `Game State Detectors`?**
