...
## Corpus Check
- 45 files · ~14,973 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 452 nodes · 573 edges · 69 communities (31 shown, 38 thin omitted)
- Extraction: 75% EXTRACTED · 25% INFERRED · 0% AMBIGUOUS · INFERRED: 141 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `df0d43cd`
...
Cohesion: 0.24
Nodes (5): WakeWordDetector, test_detect_loop_calls_on_wake_when_threshold_met(), test_detect_loop_logs_error_on_missing_import(), test_init_sets_attributes(), test_stop_sets_event()
...
- **Why does `main()` connect `Database Query Layer` to `Game State Detectors`, `Game API & HUD Capture`, `Game History Tracking`, `TTS Speech Queue`, `Message Banks & Tests`?**
  _High betweenness centrality (0.188) - this node is a cross-community bridge._
- **Why does `get_config()` connect `Game State Detectors` to `Database Query Layer`, `Game API & HUD Capture`?**
  _High betweenness centrality (0.132) - this node is a cross-community bridge._
- **Why does `GameHistoryManager` connect `Game History Tracking` to `Database Query Layer`?**
